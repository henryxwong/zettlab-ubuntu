**Guide: Custom Front LCD Status Display Service on Zettlab D6/D8 Ultra (Ubuntu 26.04)**
...
This is done **entirely after** you have finished the original Ubuntu 26.04 installation guide (Steps 1–6).  
**99 % of the work can be done via SSH** — you only need an HDMI monitor plugged in for one final verification reboot.

### 2. Prerequisites
- Ubuntu 26.04 fully installed and booted (main guide completed)
- SSH access enabled (`sudo apt install openssh-server -y && sudo systemctl enable --now ssh`)
...
#### Step 2: Fix HDMI Resolution While Keeping LCD On
```bash
...
#### Step 3: Reboot (First Time LCD + Full HDMI)
```bash
sudo reboot
```
...
### 9. Quick Management Commands (via SSH anytime)
```bash
# Restart dashboard
...
- Permission error on `smartctl` → `sudo usermod -aG disk $USER` then reboot
...
You now have a complete, production-ready custom LCD solution on plain Ubuntu 26.04.

Want me to add graphs, more drives, network speed, or switch to a Python/LVGL version instead? Just tell me what extra info you want on the LCD and I’ll update the config for you.